...
class Puzzle:
    ...
    def add_column_cages(self):
        ...
        DEBUG.print(Format.Transition, 1, "Filled %d columns" % [column for column in columns.values() if column].__len__())
        return columns
    ...
    def add_row_cages(self):
        ...
        DEBUG.print(Format.Transition, 1, "Filled %d rows" % [row for row in rows.values() if row].__len__())
        return rows
